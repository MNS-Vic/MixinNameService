...
import "../registry/MNSRegistry.sol";
...
/**
 * @dev An MNS registrar that allows the owner of a DNS name to claim the
 *      corresponding name in MNS.
 */
...
contract DNSRegistrar is IDNSRegistrar, IERC165 {
    ...
    /**
     * @dev This contract's owner-only functions can be invoked by the owner of the MNS root.
     */
    modifier onlyOwner() {
        Root root = Root(mns.owner(bytes32(0)));
    ...
    function proveAndClaim(
        ...
        mns.setSubnodeOwner(rootNode, labelHash, addr);
    }
    ...
    function proveAndClaimWithResolver(
        ...
        if (addr != address(0)) {
            ...
            mns.setSubnodeRecord(rootNode, labelHash, owner, resolver, 0);
        }
    }
        ...
        returns (bytes32 node)
    {
        ...
        if (owner != address(this)) {
            if (parentNode == bytes32(0)) {
                ...
                mns.setSubnodeOwner(parentNode, label, address(this));
            }
        }
        return node;
    }
