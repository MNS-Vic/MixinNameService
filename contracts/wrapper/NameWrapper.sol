...
import {MNS} from "../registry/MNS.sol";
    ...
    ERC20Recoverable
{
    ...
    MNS public immutable override mns;
    ...
    bytes32 private constant MIXIN_LABELHASH =
        0x00e098018205696e5a73fcee1aa73338e6bbb2a703d4cf8e725bfe228be5f7c9;
    ...
    constructor(
        ...
        /* Burn PARENT_CANNOT_CONTROL and CANNOT_UNWRAP fuses for ROOT_NODE and MIXIN_NODE */
        ...
        onlyOwner
    {
        if (address(upgradeContract) != address(0)) {
            ...
            mns.setApprovalForAll(address(upgradeContract), false);
        }
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            mns.setApprovalForAll(address(upgradeContract), true);
        }
    }
    ...
    function wrapETH2LD(
        ...
        if (
            ...
            revert Unauthorised(
                _makeNode(MIXIN_NODE, bytes32(tokenId)),
                msg.sender
            );
        }
        ...
        // transfer the mns record back to the new owner (this contract)
     ...
     * @param resolver The resolver address to set on the MNS registry (optional).
     * @param ownerControlledFuses Initial owner-controlled fuses to set
    ...
    {
        bytes32 node = _makeNode(MIXIN_NODE, bytes32(tokenId));
    ...
    function wrap(
        ...
        if (parentNode == MIXIN_NODE) {
            revert IncompatibleParent();
        }

        address owner = mns.owner(node);

        if (owner != msg.sender && !mns.isApprovedForAll(owner, msg.sender)) {
            revert Unauthorised(node, msg.sender);
        }

        if (resolver != address(0)) {
            mns.setResolver(node, resolver);
        }

        mns.setOwner(node, address(this));
    ...
    function unwrapETH2LD(
        ...
        address registrant,
        address controller
    ) public override onlyTokenOwner(_makeNode(MIXIN_NODE, labelhash)) {
        if (registrant == address(this)) {
            revert IncorrectTargetOwner(registrant);
        }
        _unwrap(_makeNode(MIXIN_NODE, labelhash), controller);
    ...
    function upgradeETH2LD(
        ...
        bytes32 node = _makeNode(MIXIN_NODE, labelhash);
    ...
    function setChildFuses(
        ...
        if (owner == address(0) || mns.owner(node) != address(this)) {
    ...
    function setSubnodeOwner(
        ...
        if (!isWrapped(node)) {
            mns.setSubnodeOwner(parentNode, labelhash, address(this));
    ...
    /**
     * @notice Sets records for the name in the MNS Registry
    ...
    {
        mns.setRecord(node, address(this), resolver, ttl);
        if (owner == address(0)) {
    ...
    function _wrapETH2LD(
        ...
        bytes memory name = _addLabel(label, "\x03mixin\x00");
        names[node] = name;
